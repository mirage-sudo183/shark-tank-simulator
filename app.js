...
(function() {
  ...
  /**
   * Check if user is returning from a magic link email
   */
  async function checkMagicLinkSignIn() {
    if (!window.firebaseAuth || !window.firebaseAuth.isSignInWithEmailLink) return;

    try {
      if (window.firebaseAuth.isSignInWithEmailLink()) {
        const result = await window.firebaseAuth.completeMagicLinkSignIn();
        if (result) {
  ...
  function setupAuthButtons() {
    ...
    if (googleLoginBtn) {
      googleLoginBtn.addEventListener('click', async () => {
        if (!window.firebaseAuth || !window.firebaseAuth.signInWithGoogle) {
          showAuthError('Google sign-in is not available.');
          return;
        }
        try {
          clearAuthMessages();
    ...
    // Email auth tabs
    const emailTabs = document.querySelectorAll('.auth-email-tab');
    emailTabs.forEach(tab => {
      tab.addEventListener('click', () => {
        clearAuthMessages();
        // Update active tab
        emailTabs.forEach(t => t.classList.remove('auth-email-tab--active'));
        tab.classList.add('auth-email-tab--active');

        // Show corresponding form
        const tabName = tab.dataset.tab;
        document.getElementById('emailSignInForm').style.display = tabName === 'signin' ? 'block' : 'none';
        document.getElementById('emailRegisterForm').style.display = tabName === 'register' ? 'block' : 'none';
        document.getElementById('magicLinkForm').style.display = tabName === 'magic' ? 'block' : 'none';
      });
    });
    ...
    if (emailSignInForm) {
      emailSignInForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        if (!window.firebaseAuth || !window.firebaseAuth.signInWithEmail) {
          showAuthError('Email sign-in is not available.');
          return;
        }
    ...
    if (emailRegisterForm) {
      emailRegisterForm.addEventListener('submit', async (e) => {
        ...
        if (!window.firebaseAuth || !window.firebaseAuth.registerWithEmail) {
          showAuthError('Email registration is not available.');
          return;
        }
        ...
        try {
          ...
          if (result.user && window.firebaseAuth.updateProfile) {
    ...
    if (magicLinkForm) {
      magicLinkForm.addEventListener('submit', async (e) => {
        e.preventDefault();
        if (!window.firebaseAuth || !window.firebaseAuth.sendMagicLink) {
          showAuthError('Magic link sign-in is not available.');
          return;
        }
        ...
        try {
          ...
          showAuthSuccess('Magic link sent! Check your email inbox.');
          submitBtn.disabled = false;
